    ...
    try {
      ...
      if (user == null) {
        FirebaseCrashlytics.instance.setCustomKey(
          'ProductInfo',
          event.cartList.first.product.title,
        );
        FirebaseCrashlytics.instance.log("[PaymenyError] No User Exception");

        emit(
          state.copyWith(status: PaymentStatus.error, message: '로그인 후 이용해주세요'),
        );
...
Payload _getPayLoad(List<Cart> cartList, User? loginUser) {
  ...
  payload.androidApplicationId = dotenv.env['BOOTPAY_ANDROID_APP_ID']!;
  payload.iosApplicationId = dotenv.env['BOOTPAY_IOS_APP_ID']!;
