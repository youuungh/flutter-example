...
// 상품 데이터
List<ProductInfoDto> storeProductList = [
  ProductInfoDto(
...
// viewModules
List<ViewModuleDto> viewModules(String storeType) {
  return [
    ...
    ViewModuleDto(
      ...
      title: '이 상품 어때요?',
      products: productGenerator(10, storeType),
    ),
    ViewModuleDto(
      type: 'special_price_view_module',
      title: '24시간 특가',
      subtitle: '24시간 동안만 만날 수 있는 특별한 가격!',
      products: productGenerator(4, storeType),
      time: DateTime
          .now()
    ...
    ViewModuleDto(
      ...
      title: 'MD의 추천',
      products: productGenerator(6, storeType),
      tabs: [
        '가전, 가구 특가',
        '프리미엄 주방 특가',
        '뷰티 특가',
    ...
    ViewModuleDto(
      ...
      products: productGenerator(3, storeType),
    ),
    ViewModuleDto(
      type: 'banner_view_module',
      imageUrl: bannerImageList[Random().nextInt(5)],
    ...
    ViewModuleDto(
      type: 'scroll_view_module',
      title: '지금 가장 핫한 상품',
      products: productGenerator(7, storeType),
    ),
    ViewModuleDto(
      type: 'scroll_view_module',
      title: '가성비 최고의 상품들',
      subtitle: '100g 당 가격으로 환산해보면 진짜 저렴해요!',
      products: productGenerator(5, storeType),
    ),
  ];
}

// menu data
List<Map<String, Object>> storeMenus = [
  {"tabId": 10001, "title": "추천"},
...
List<Map<String, Object>> beautyMenus = [
  {"tabId": 20001, "title": "추천"},
...
List<ProductInfoDto> productGenerator(int length, String storeType) {
  ...
  var productList =
  storeType == 'store' ? storeProductList : beautyProductList;

  List<ProductInfoDto> list = List.generate(
    length,
        (index) {
      int originalPrice = (Random().nextInt(31) + 1) * 1000;
      int saleRate = Random().nextInt(50) + 1;

      return productList[randomSet.elementAt(index)].copyWith(
        productId:
        '$originalPrice'.padLeft(6, '0') + '$saleRate'.padLeft(6, '0'),
        price: originalPrice * (100 - saleRate) ~/ 100,
        saleRate: saleRate,
...
Set<int> getRandomSet(int length, int max) {
  ...
  while (random.length != length) {
    random.add(Random(DateTime
        .now()
        .microsecondsSinceEpoch).nextInt(max));
  }
